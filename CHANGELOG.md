...
- Audio pipeline (whisper build): PortAudio capture, WebRTC VAD, whisper.cpp transcription, wake word (“clawd”).
- Mic management (`list-mics`, `set-mic`), model management (`models list|download|set`), setup downloads default model.
...
- Rich Go help output with examples; pnpm scripts for build/run.
